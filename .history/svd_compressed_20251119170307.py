...
# ฟังก์ชันหาค่า U, σ, V จาก A^T A (ตามทฤษฎี SVD)
def svd_from_eigendecomp(A: np.ndarray, eps: float = 1e-12):
    ...
    # เรียง sigma จากมากไปน้อย พร้อมเรียงคอลัมน์ของ V ตาม
    idx = np.argsort(sigma)[::-1]
    sigma = sigma[idx]
    V = V[:, idx]

    # ตัดค่า sigma ที่เล็กมาก ๆ ทิ้ง (กันเลขเพี้ยน)
    r = int(np.sum(sigma > eps))
    sigma_r = sigma[:r]
    V_r = V[:, :r]

    # 3) หา U จาก U = A V Σ^{-1}
    U_r = A @ V_r
    U_r = U_r / sigma_r[None, :]

    # ปรับให้แต่ละคอลัมน์ของ U มี norm = 1
    U_r = U_r / np.linalg.norm(U_r, axis=0, keepdims=True)

    return U_r, sigma_r, V_r


# ฟังก์ชันสร้าง A_k (rank-k approximation)
def reconstruct_rank_k(U_r, sigma_r, V_r, k: int):
    k_eff = int(min(k, len(sigma_r)))
    if k_eff == 0:
        return np.zeros((U_r.shape[0], V_r.shape[0]), dtype=np.float64), 0
...
# ลองหลายค่า k เพื่อดูความชัดที่ต่างกัน
k_list = [1, 2, 4, 100]
compressed_list = []

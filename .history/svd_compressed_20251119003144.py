...
# -----------------------------
# 1) คำนวณ SVD จาก A^T A (ตามสูตรทฤษฎี)
# -----------------------------
def svd_from_eigendecomp(A: np.ndarray, eps: float = 1e-12):
    # 1. หา eigenvalues / eigenvectors ของ A^T A
    AtA = A.T @ A
    lam, V = np.linalg.eigh(AtA)          # eigen-decomposition

    # 2. singular values = sqrt(lambda)
    sigma = np.sqrt(np.clip(lam, 0, None))
    ...
    # ตัดค่าเล็ก ๆ ที่ใกล้ 0 ทิ้ง
    r = int(np.sum(sigma > eps))
    sigma_r = sigma[:r]
    V_r = V[:, :r]

    # 3. หา U จาก U = A V / sigma
    U_r = A @ V_r
    U_r = U_r / sigma_r[None, :]          # แบ่งแต่ละคอลัมน์ด้วย sigma_i

    # ทำให้แต่ละคอลัมน์ของ U มี norm = 1
    U_r = U_r / np.linalg.norm(U_r, axis=0, keepdims=True)

    return U_r, sigma_r, V_r

# -----------------------------
# 2) สร้างเมทริกซ์ rank-k: A_k = sum_{i=1..k} sigma_i u_i v_i^T
# -----------------------------
def reconstruct_rank_k(U_r, sigma_r, V_r, k: int):
    # ใช้จริงแค่ min(k, จำนวน singular values)
    ...
    for i in range(k_eff):
        ui = U_r[:, i][:, None]       # column vector (m x 1)
        vi = V_r[:, i][None, :]       # row vector (1 x n)
        Ak += sigma_r[i] * (ui @ vi)  # rank-1 term
...
# 4) ลองหลายค่า k
# -----------------------------
k_list = [5, 20, 50, 100]   # อยากลองค่าไหนเพิ่ม/ลด แก้ list นี้ได้เลย
